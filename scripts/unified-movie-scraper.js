...
const CONFIG = {
    ...
    timeout: 15000,
    headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/122.0',
...
async function loadImdbBasics() {
    ...
    for await (const line of rl) {
        ...
        const [tconst, titleType, primaryTitle, originalTitle, isAdult, startYear, , runtimeMinutes, genres] = cols;
...
async function main() {
    ...
    console.log(`Options: skipEnrich=${options.skipEnrich}, forceHtml=${options.forceHtml}, allTimeLimit=${options.allTimeLimit}`);
    ...
    const topPayload = buildPayload({
        ...
        methodology: 'Weighted IMDb ratings; TSV ranking replicated client-side using rating, votes, tie-breakers.',
    ...
    console.log(`   Top250 added ${topDiff.added}, removed ${topDiff.removed}, changed ${topDiff.changed}`);
    console.log(`   AllTime added ${allDiff.added}, removed ${allDiff.removed}, changed ${allDiff.changed}`);
}
