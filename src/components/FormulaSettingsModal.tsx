...
export const FormulaSettingsModal: React.FC<FormulaSettingsModalProps> = ({
  ...
  const playlistStats = useMemo(() => {
    ...
    const median =
      views.length % 2 !== 0 ? views[mid] : Math.round((views[mid - 1] + views[mid]) / 2);
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto max-h-[70vh]">
              ...
              {currentStats && (
                <div className="animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-zinc-800 rounded-lg p-3 border border-zinc-700/50">
                      ...
                      <div className="text-xl font-bold text-white font-mono">
                        {formatViews(currentStats.median)}
                      </div>
                    </div>
                    <div className="bg-zinc-800 rounded-lg p-3 border border-zinc-700/50">
                      <div className="text-xs text-zinc-500 mb-1">Среднее (Mean)</div>
                      <div className="text-xl font-bold text-blue-400 font-mono">
                        {formatViews(currentStats.avg)}
                      </div>
                    </div>
                    <div className="bg-zinc-800 rounded-lg p-3 border border-zinc-700/50">
                      <div className="text-xs text-zinc-500 mb-1">Максимум</div>
                      <div className="text-lg font-medium text-zinc-300 font-mono">
                        {formatViews(currentStats.max)}
                      </div>
                    </div>
                    <div className="bg-zinc-800 rounded-lg p-3 border border-zinc-700/50">
                  ...
                  <p className="text-[10px] text-zinc-500 mt-2 text-center">
                    Используйте медиану {activeStatsTab === 'global' ? 'базы' : 'плейлиста'} как
                    ориентир для настройки порога "7.0 баллов"
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="mb-6 bg-zinc-800/50 p-3 rounded-lg flex items-center justify-between border border-zinc-700">
            <div className="flex items-center gap-3">
              <div
                className={`p-2 rounded-lg ${tempSettings.useExperimentalStrategy ? 'bg-purple-600' : 'bg-zinc-700'}`}
              >
            ...
            <button
              type="button"
              onClick={() =>
                handleChange('useExperimentalStrategy', !tempSettings.useExperimentalStrategy)
              }
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-zinc-500 mt-2 leading-relaxed">
                    <strong>Стартовая точка.</strong> Это оценка, которую получает видео "по
                    умолчанию", пока не наберет дополнительных баллов за активность. Например, если
                    выставить 5.0, то даже самое непопулярное видео не будет иметь рейтинг ниже 5.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500 mt-2 leading-relaxed">
                    ...
                    <br />
                    • Высокое значение: Рейтинг быстро растет даже при небольшом увеличении
                    просмотров.
                    <br />• Низкое значение: Требуется огромное количество просмотров, чтобы поднять
                    рейтинг выше базового.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="p-4 bg-zinc-800/20 rounded-lg border border-zinc-800/50">
                  ...
                  <div className="mb-4">
                    <label className="flex justify-between text-xs text-zinc-400 mb-1.5">
                      ...
                      <span className="text-white font-bold">
                        {tempSettings.targetRatingLow ?? 7.0}
                      </span>
                    </label>
                    <input
                      type="range"
                  ...
                  <div className="relative">
                    ...
                    />
                    {tempSettings.useMedianForLow && (
                      <Lock className="absolute right-3 top-8 w-4 h-4 text-zinc-600" />
                    )}
                  </div>
                </div>
                ...
                <div className="p-4 bg-zinc-800/20 rounded-lg border border-zinc-800/50">
                  ...
                  <div className="mb-4">
                    <label className="flex justify-between text-xs text-zinc-400 mb-1.5">
                      <span>Целевой рейтинг</span>
                      <span className="text-white font-bold">
                        {tempSettings.targetRatingHigh ?? 9.0}
                      </span>
                    </label>
                    <input
                      type="range"
          ...
          <div className="mb-4">
            ...
            <div className="space-y-6">
              <div>
                ...
                />
                <p className="text-xs text-zinc-500 mt-2 leading-relaxed">
                  Добавляется к возрасту видео. Предотвращает ситуацию, когда только что вышедшее
                  видео с 1 просмотром получает бесконечно огромный рейтинг.
                </p>
              </div>
              ...
              <div>
                ...
                />
                <p className="text-xs text-zinc-500 mt-2 leading-relaxed">
                  Скорость "остывания". Чем выше значение, тем быстрее видео теряет позицию в
                  трендах со временем.
                </p>
              </div>
            </div>
