...
export const KinoRateModal: React.FC<KinoRateModalProps> = ({
  initialQuery,
  contextKey,
  onClose,
  onSaveMetadata,
  ...
  const getTopListData = useMemo(() => {
    const base = mode === 'top900' ? TOP_IMDB_MOVIES : TOP_250_MOVIES;
                    ...
                    <div className="p-6 md:p-8 flex flex-col md:flex-row gap-6 md:items-start">
                      ...
                      <div className="flex-1 text-center md:text-left pt-2">
                        ...
                        <p className="text-zinc-400 text-lg mb-4">
                          {singleResult.originalTitle} ({singleResult.year})
                        </p>
                        ...
                        <div className="flex flex-wrap justify-center md:justify-start gap-4">
                          <div className="flex items-center gap-3 bg-[#f60]/10 border border-[#f60]/30 px-4 py-2 rounded-xl">
                            <div className="text-[#f60] font-bold text-xl">
                              {singleResult.kpRating}
                            </div>
                        ...
                        {singleResult.awards && singleResult.awards.length > 0 && (
                          <div className="mt-6 flex flex-wrap gap-2 justify-center md:justify-start">
                            {singleResult.awards.map((award, i) => (
                              <div
                                key={i}
                                className="px-2 py-1 bg-yellow-500/10 border border-yellow-500/30 rounded text-[10px] text-yellow-200 flex items-center gap-1"
                              >
              ...
              {!isBatchProcessing && batchQueue.length === 0 ? (
                <div className="flex-1 flex flex-col gap-4">
                  <p className="text-zinc-400 text-sm">
                    Вставьте список фильмов (каждый с новой строки) для массовой проверки.
                  </p>
                  <textarea
                    value={batchInput}
                ...
                <div className="flex-1 flex flex-col">
                  <div className="flex items-center justify-between mb-4">
                    <div className="text-sm text-zinc-400">
                      Обработано: <span className="text-white font-bold">{processedCount}</span> из{' '}
                      {batchQueue.length}
                    </div>
                    {!isBatchProcessing && (
                      <div className="flex gap-2">
                        <button
                          onClick={exportCSV}
                          className="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-white rounded-lg text-xs font-medium flex items-center gap-2"
                        >
                  ...
                  <div className="flex-1 overflow-y-auto pr-2 space-y-2">
                    {batchQueue.map(item => (
                      <div
                        key={item.id}
                        className="bg-zinc-900/50 border border-zinc-800 p-3 rounded-lg flex items-center gap-4"
                      >
            ...
            <div className="h-full flex flex-col">
                  ...
                  <table className="w-full text-left text-sm text-zinc-400">
                    ...
                    <tbody className="divide-y divide-zinc-800">
                      {displayedListItems.map((movie, idx) => {
                        ...
                        const isOscarNominee =
                          !isOscarWinner &&
                          oscarAwards.some(
                            a => a.status === 'Nominated' || a.status === 'Nominee' || !a.status
                          );
                            ...
                            <td className="px-4 py-3 text-white">
                              <div className="flex items-center gap-2 min-w-0">
                                ...
                                {mode === 'top250' ? (
                                  <a
                                    href={
                                      movie.imdbUrl || `https://www.imdb.com/title/tt${movie.id}/`
                                    }
                                    target="_blank"
                                  ...
                                  <a
                                    href={
                                      movie.top250Url || `http://top250.info/movie/?${movie.id}`
                                    }
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    onClick={e => e.stopPropagation()}
              ...
              {totalListPages > 1 && (
                <div className="flex justify-between items-center mt-4 text-xs text-zinc-500">
                  ...
                  <span>
                    Страница {listPage} из {totalListPages}
                  </span>
                  <button
